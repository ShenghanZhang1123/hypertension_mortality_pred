...
from allmodel import lstm, rnn, mlp, DLClassifier
...
def subset_data(df, label, k=100):
    pos_rate = sum(label == 1.0)/len(label)
    num_pos = int(np.ceil(k*pos_rate))
    num_neg = k - num_pos

    pos_df = shap.utils.sample(df[label == 1.0], num_pos)
    neg_df = shap.utils.sample(df[label == 0.0], num_neg)
...
def cross_validation(model, x, y, k=5):
    ...
    for train_index, val_index in kf.split(x):
        ...
        pred_prob = model.predict_prob(x_val)
...
if __name__ == '__main__':
    if args.label_balance and args.test_balance:
        result_path = './stats_ehr_result/dl/' + 'train_test_balanced/' + args.disease
    elif args.label_balance:
        result_path = './stats_ehr_result/dl/' + 'train_balanced/' + args.disease
    elif args.test_balance:
        result_path = './stats_ehr_result/dl/' + 'test_balanced/' + args.disease
    else:
        result_path = './stats_ehr_result/dl/' + 'no_balanced/' + args.disease
    ...
    X_BG = subset_data(X_train, y_train, 200)
    X_shap = subset_data(X_test, y_test, 200)

    if args.model.lower() not in ['lstm', 'rnn', 'mlp']:
        print('Not supported model. Please choose from the following four machine learning models: {}'.format(
            str(['lstm', 'rnn', 'mlp'])))
        sys.exit(1)
    elif args.model.lower() == 'lstm':
        model = lstm(len(X.columns))
        classifier = DLClassifier(model, args.lr, args.minibatch, args.num_epoch, cuda=args.cuda, verbose=args.verbose)
    elif args.model.lower() == 'rnn':
        model = rnn(len(X.columns))
        classifier = DLClassifier(model, args.lr, args.minibatch, args.num_epoch, cuda=args.cuda, verbose=args.verbose)
    else:
        model = mlp(len(X.columns))
        classifier = DLClassifier(model, args.lr, args.minibatch, args.num_epoch, cuda=args.cuda, verbose=args.verbose)
    ...
    metrics_list = list(
        map(lambda x: x + '_feature_' + str(X_test.shape[1]), ['Acc', 'AUC', 'Recall', 'Precision', 'F1'])) \
    ...
    pred_prob = classifier.predict_prob(X_test)
    ...
    if args.explainer == 'kernel':
        X_array, X_BG_array, X_shap_array = X.to_numpy(), X_BG.to_numpy(), X_shap.to_numpy()

        explainer = shap.KernelExplainer(classifier.predict_prob, X_BG_array, feature_names=X_BG.columns)
        explanation = explainer(X_shap_array)
        shap_values = explanation.values
    else:
        X_array, X_BG_array, X_shap_array = torch.from_numpy(X.to_numpy()), torch.from_numpy(
            X_BG.to_numpy()), torch.from_numpy(X_shap.to_numpy())

        if args.cuda:
            X_array, X_BG_array, X_shap_array = X_array.cuda(), X_BG_array.cuda(), X_shap_array.cuda()
        # Create a SHAP explainer object
        #explainer = shap.DeepExplainer(classifier.model.train(), X100_array)
        explainer = shap.GradientExplainer(classifier.model, X_BG_array)
        X_shap_array = torch.from_numpy(X_test).cuda()
        # Calculate SHAP values for the test set
        shap_values = explainer.shap_values(X_shap_array)[1]
        base_value = np.mean(classifier.predict_prob(X_BG_array))

        explanation = shap.Explanation(values=shap_values,
                                       base_values=base_value,
                                       data=X_shap_array.cpu().numpy(),
    ...
    shap.plots.bar(explanation, max_display=30, show=False)
    plt.tight_layout()
    plt.savefig(fig_path)
    plt.close()
    ...
    top_indices = shap_values_abs_mean.argsort()[::-1][:args.top]  # Indices of top n features
    rank_indices = shap_values_abs_mean.argsort()[::-1]

    shape_value_df = pd.DataFrame({'Feature_names': X.columns[rank_indices].to_list(), 'Shape_values': list(shap_values_abs_mean[rank_indices])})
    feature_30 = X.columns[top_indices].to_list()
    ...
    cross_validation(classifier, X_train[:, top_indices], y_train, 5)
    classifier.fit(X_train[:, top_indices], y_train, X_test[:, top_indices], y_test)

    pred = classifier.predict(X_test[:, top_indices])
    pred_prob_r = pred_prob
    pred_prob = classifier.predict_prob(X_test[:, top_indices])
    ...
    pd.DataFrame({'Metrics': metrics_list, 'Values': metric_values}).to_csv(
        os.path.join(result_path, "result_" + args.model + "_" + args.task + ".csv"))
...
'''
    if args.model.lower() not in ['lstm', 'rnn', 'mlp']:
        ...
        model = Sequential()
        model.add(Embedding(len(X.columns), 128))
        ...
        model.add(Dense(128, dropout=0.2, recurrent_dropout=0.2, activation="relu"))
        model.add(Dense(1, activation="sigmoid"))
